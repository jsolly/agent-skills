...
This is the template the orchestrator (`/ship`) uses when invoking each reviewer subagent via the `Task` tool. The orchestrator fills the placeholders before dispatching.
...
| `{GUIDELINES}` | Concatenated content of project root `AGENTS.md`, any `AGENTS.md` in directories containing modified files, and the active global agent brief (desktop: read `~/.cursor/AGENTS.md` symlink target; app repos: `.agents/AGENTS.md`) | The user's installed AGENTS.md brief alone if no project `AGENTS.md` exists |
...
If multiple harness plan files were modified within the session window:
...
3. **If no plans exist**: fall through to recent-message-referenced paths, then to the conversation's first user message describing intent, then to the literal sentinel.
