...
**This is the canonical output format every review subagent must follow.** Every agent in your installed `agents/` directory inlines this contract; local installs expose those prompts through `~/.cursor/agents/*.md` symlinks, and app repos receive them at `.agents/agents/`. The `/ship` orchestrator consumes this format. Drift across agents breaks the aggregator.
