...
# Health check is informational only — best-effort, never let it abort the crawl. The `|| true`
# keeps a failing `self doctor` probe from killing the script under `set -e` before
# `squirrel audit` ever runs.
"$squirrel_bin" self doctor >&2 || true
