...
const usage = `Usage: gsc-inspect.mts --site-url <property-url> (--url <url> | --url-file <file> | --stdin)
...
function parseArgs(args: string[]): ParsedArgs {
  const parsed: ParsedArgs = { urls: [], max: 200, delayMs: 100 };
  for (let index = 0; index < args.length; index += 1) {
    const arg = args[index];
    if (arg === "--help" || arg === "-h") {
      parsed.help = true;
    } else if (arg === "--site-url") {
      parsed.siteUrl = args[(index += 1)];
    } else if (arg === "--url") {
      ...
      parsed.urlFile = args[(index += 1)];
    } else if (arg === "--stdin") {
      parsed.stdin = true;
    } else if (arg === "--max") {
      parsed.max = Number.parseInt(args[(index += 1)] ?? "", 10);
...
async function getAccessToken(): Promise<string> {
  ...
  if (env.GOOGLE_APPLICATION_CREDENTIALS) {
    const credentials = JSON.parse(await readFile(env.GOOGLE_APPLICATION_CREDENTIALS, "utf8")) as {
      client_email?: string;
      private_key?: string;
    };
    ...
    if (!token.access_token) {
      fail("Google token endpoint returned no access_token");
    }
    return token.access_token;
  }

  try {
    const { stdout: tokenOut } = await execFileAsync("gcloud", [
      "auth",
      "application-default",
      "print-access-token",
    ]);
...
async function resolveQuotaProject(): Promise<string | undefined> {
  ...
  try {
    const adc = JSON.parse(await readFile(adcPath, "utf8")) as { quota_project_id?: string };
...
async function inspectUrl(params: {
  token: string;
  quotaProject: string | undefined;
  siteUrl: string;
  url: string;
}): Promise<InspectResult> {
  const { token, quotaProject, siteUrl, url } = params;
...
async function main(): Promise<void> {
  ...
  if (args.checkAuth) {
    ...
    // The service-account and gcloud paths mint a token (a real validation); a pre-supplied
    // GSC_ACCESS_TOKEN is used as-is and never checked, so don't overstate its readiness.
    stdout.write(
      env.GSC_ACCESS_TOKEN
        ? "GSC access token present (env GSC_ACCESS_TOKEN used as-is — not validated)\n"
        : "GSC auth ready\n",
    );
    return;
  }
  ...
  stdout.write(`${JSON.stringify({ siteUrl: args.siteUrl, count: results.length, results }, null, 2)}\n`);

  // Emit the detail above, then fail loud if EVERY inspection failed — an expired token or wrong
  // quota project 403s all URLs, and exiting 0 would let a caller read broken auth as "clean".
  if (results.every((result) => !result.ok)) {
    fail(
      `All ${results.length} GSC inspection(s) failed (first status ${results[0]?.status ?? "?"}); ` +
        "check the token, quota project, and property permission",
    );
  }
}
